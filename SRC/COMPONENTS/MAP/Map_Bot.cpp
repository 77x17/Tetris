...
int Map_Bot::getHeuristicScore(uint16_t shape, uint8_t shapeID, int X, int Y) {
    if (!isValid(shape, Y, X)) return INT_MIN;

    while (isValid(shape, Y + 1, X)) Y++;
    return botBrain->getHeuristicScore(shape, shapeID, X, Y);
}
...
void Map_Bot::findPath(int8_t &X, int8_t &timeRotate, bool &hold, CurrentBlock_Bot* a, CurrentBlock_Bot* b) {
    ...
    for (int i = 0; i <= 5; i++) {
        for (int j = 0; j < 4; j++) for (int t: {-1, 1}) {
            // std::cout << isHold << '\n';
            int tmp = getHeuristicScore(curBlock[isHold]->getShape(j), curBlock[isHold]->getShapeID(), MID + i*t, 0);
            if (maxScore < tmp) {
                maxScore = tmp;
                X = MID + i*t;
                timeRotate = j;
